...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    @IBInspectable dynamic open var lineColor: UIColor = .clear {
        didSet {
            updateLineView()
        }
    }
    ...
    @IBInspectable dynamic open var errorColor: UIColor = .clear {
        didSet {
            updateColors()
        }
    }
    ...
    /// A UIColor value that determines the color of the border when in the normal state
    ...
    /// A UIColor value that determines the color used for the border when error message is not `nil`
    ...
    @IBInspectable dynamic open var selectedLineColor: UIColor = .clear {
        didSet {
            updateLineView()
        }
    }
    ...
    @IBInspectable dynamic open var lineHeight: CGFloat = 0.0 {
        didSet {
            updateLineView()
            setNeedsDisplay()
        }
    ...
    */
    ...
    fileprivate func setLeftView() {
        let leftView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: 15.0, height: self.frame.height))
    ...
    */
    ...
    fileprivate func updateTitleLabel(_ animated: Bool = false) {
        ...
        if hasErrorMessage {
            titleText = errorMessage!   //titleFormatter(errorMessage!)
        } else {
    ...
    */
    ...
    override open func placeholderRect(forBounds bounds: CGRect) -> CGRect {
        let widthLeftView = leftView?.bounds.width ?? 0
        let rect = CGRect(
            x: widthLeftView,
            y: 0, //titleHeight(),
    ...
    */
    open func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        let widthLeftView = leftView?.bounds.width ?? 0
        if editing {
            return CGRect(x: widthLeftView, y: 5, width: bounds.size.width, height: titleHeight())
        }
        return CGRect(x: widthLeftView, y: titleHeight(), width: bounds.size.width, height: titleHeight())
    }
    ...
    fileprivate func titleOrPlaceholder() -> String? {
        ...
        return title    //titleFormatter(title)
    }
